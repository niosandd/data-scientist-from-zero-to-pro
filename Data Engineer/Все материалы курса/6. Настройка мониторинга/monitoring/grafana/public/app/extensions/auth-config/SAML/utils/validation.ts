...
import { type SAMLFormData, SAMLStepKey } from '../../../types';
...
function validateConfigGeneral(config: SAMLFormData): ValidationResult {
  const validationRes: ValidationResult = { valid: true, errors: [] };
...
function validateConfigKeyCert(config: SAMLFormData): ValidationResult {
  const validationRes: ValidationResult = { valid: true, errors: [] };
...
function validateConfigConnectToIdP(config: SAMLFormData): ValidationResult {
  const validationRes: ValidationResult = { valid: true, errors: [] };
...
function validateConfigAssertionMapping(config: SAMLFormData): ValidationResult {
  const validationRes: ValidationResult = { valid: true, errors: [] };
  return validationRes;
}
...
function checkPKCS8Format(data: string): boolean {
...
function validatePEMCert(cert: string): boolean {
  // FIXME: should return the error message to use in the UI
  // this would help the user to understand what is wrong with the cert
  // that is uploaded
  try {
