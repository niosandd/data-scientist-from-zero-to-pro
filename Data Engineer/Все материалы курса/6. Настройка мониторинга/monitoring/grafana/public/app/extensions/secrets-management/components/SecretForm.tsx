...
import { type ChangeEvent, type FormEvent, useState, useEffect, useId } from 'react';
...
import { type FieldErrorMap, type SecretFormValues } from '../types';
import {
...
export function SecretForm({
  ...
  const isNew = initialValues?.name === undefined;
  ...
  const maxLabelsReached = labels && labels.length >= SECRETS_MAX_LABELS;
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} data-testid={selectors.pages.SecretsManagement.SecretForm}>
        ...
        >
          <Controller
            ...
            render={({ field: { ref, ...field } }) => (
              // TSGO / TS7 is much stricter with generics. Whilst we're between TS versions, we need to explicitly define the generic type here to avoid errors.
              <MultiSelect<string>
