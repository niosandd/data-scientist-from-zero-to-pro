...
const enrichmentHandlers = [
  ...
  http.get('/apis/alertenrichment.grafana.app/v1beta1/namespaces/default/alert-enrichments', ({ request }) => {
    ...
    if (labelSelector) {
      if (labelSelector === 'enrichment-scope=global') {
        // Globally scoped = no labelMatchers and no annotationMatchers (scope "Global scoped" / all alerts). Only these show in the rule page global section.
          ...
          mockAlertEnrichmentList([
            mockAlertEnrichment({
              ...
              spec: {
                ...
                steps: [
                  { timeout: '30s', type: 'enricher', enricher: { type: 'assign', assign: { annotations: [] } } },
                ],
                labelMatchers: [],
                annotationMatchers: [],
              },
  ...
  // POST notification/query - Historian notification list (used by EnrichmentPreviewSection to load notification history for preview)
  http.post('/apis/historian.alerting.grafana.app/v0alpha1/namespaces/default/notification/query', () =>
    HttpResponse.json({ entries: [] })
  ),

  // POST enricher/preview - Preview enrichment (request body: enrichmentSpec + notificationPayload)
  http.post('/apis/alertenrichment.grafana.app/v1beta1/namespaces/default/enricher/preview', async ({ request }) => {
    const body = (await request.json()) as {
      enrichmentSpec?: { title?: string };
      notificationPayload?: unknown[];
    };
    const alerts = body?.notificationPayload ?? [];
    return HttpResponse.json({
      enrichedPayload: alerts,
      previewTitle: body?.enrichmentSpec?.title ?? 'Preview',
    });
  }),
];
