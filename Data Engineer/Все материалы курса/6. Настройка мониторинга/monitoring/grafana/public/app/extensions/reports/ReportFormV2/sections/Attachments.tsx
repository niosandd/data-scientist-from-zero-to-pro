...
import { csvEncodingOptions, reportOrientations, reportLayouts, type ReportFormV2 } from '../../../types';
    ...
    >
      <Stack direction="column" alignItems={'start'} gap={1.5}>
        <Checkbox
          ...
          data-testid={selectors.components.ReportFormDrawer.Attachments.pdfReportCheckbox}
        />
        {isPDFTablesEnabled && (
          <Checkbox
            label={t('share-report.attachments.pdf-tables', 'Attach a separate PDF of table data')}
            {...register('attachments.pdfTables')}
            data-testid={selectors.components.ReportFormDrawer.Attachments.pdfTablesCheckbox}
          />
        )}
        <Checkbox
          label={t('share-report.attachments.csv-tables', 'Attach a CSV file of table panel data')}
          {...register('attachments.csv')}
          data-testid={selectors.components.ReportFormDrawer.Attachments.csvTablesCheckbox}
        />
        {isCSVSelected && (
                ...
                <Stack direction="column" alignItems={'start'} gap={1.5}>
                  {dashboards && dashboards?.length > 1 && (
                    <Checkbox
                      ...
                      data-testid={selectors.components.ReportFormDrawer.Attachments.combineDashboardsCheckbox}
                    />
                  )}
                  {hasDashboardWithVariables && (
                    <Checkbox
                      ...
                      data-testid={selectors.components.ReportFormDrawer.Attachments.showTemplateVariablesCheckbox}
                    />
                  )}
                  {isPDFTablesEnabled && (
                    <Checkbox
                      ...
                      data-testid={selectors.components.ReportFormDrawer.Attachments.pdfTablesAppendixCheckbox}
                    />
                  )}
                </Stack>
              </div>
